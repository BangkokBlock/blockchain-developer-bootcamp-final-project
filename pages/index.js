...
export default function Home() {
  ...
  async function loadNFTs() {
    ...
    const marketContract = new ethers.Contract(process.env.MARKET_ADDRESS, MVMarket.abi, provider)
  ...
  async function buyNFT(nft) {
    ...
    const contract = new ethers.Contract(process.env.TOKEN_ADDRESS, MVMarket.abi, signer)
    const price = ethers.utils.parseUnits(nft.price.toString(), 'ether')
    const transaction = await contract.createMarketSale(process.env.TOKEN_ADDRESS, nft.tokenId, {
      value: price
    })
